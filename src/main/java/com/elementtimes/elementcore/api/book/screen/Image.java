...
public class Image extends BaseContent {
    ...
    public int draw(int x, int y, int spaceX, int spaceY, int mouseX, int mouseY) {
        ...
        Minecraft.getInstance().getTextureManager().bindTexture(mTexture);
        x += mOffsetX;
        if (mTextureWidth > 0 && mTextureHeight > 0) {
            AbstractGui.blit(x, y, mU, mV, mWidth, mHeight, mTextureWidth, mTextureHeight);
        } else {
            container.blit(x, y, mU, mV, mWidth, mHeight);
        }
        return y + mHeight;
    }
