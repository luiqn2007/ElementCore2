...
public class Text extends BaseContent {
    ...
    public int draw(int x, int y, int spaceX, int spaceY, int mouseX, int mouseY) {
        ...
        int textHeight = mc.fontRenderer.getWordWrappedHeight(text, textWidth);
        x += mOffsetX;
        spaceX -= mOffsetX;
        if (spaceX >= textWidth && spaceY >= textHeight) {
            mc.fontRenderer.drawString(text, x, y, mColor);
            return y + textHeight;
        }
        // width
        String[] lines;
        if (textWidth > spaceX) {
            ...
            for (String s : split) {
                ...
                while (end < s.length()) {
                    char c = s.charAt(end);
                    length += mc.fontRenderer.getCharWidth(c);
                    end++;
                    if (length >= spaceX) {
        ...
        for (String s : lines) {
            if (display) {
                int subTextHeight = mc.fontRenderer.getWordWrappedHeight(s, mc.fontRenderer.getStringWidth(s));
                if (subTextHeight <= spaceY) {
                    mc.fontRenderer.drawString(s, x, y, mColor);
