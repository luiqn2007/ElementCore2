...
import com.elementtimes.elementcore.api.interfaces.invoker.*;
    ...
    public static <T> Supplier<T>       getter(@Nonnull ECModElements elements, @Nullable Object getter, @Nonnull Class<? extends T> objType) {
        ...
        if (optional.isPresent()) {
            ...
            if (name == null || "<init>".equals(name)) {
                return () -> ReflectUtils.create(aClass, objType, elements).orElse(null);
            } else if ("()".startsWith(name)) {
                return () -> ReflectUtils.invoke(name.substring(2), aClass, null, objType, elements).orElse(null);
            } else {
                return () -> ReflectUtils.get(aClass, name, null, objType, elements).orElse(null);
            }
        } else {
            return () -> null;
        }
    }
    public static <T> Supplier<T> getterOrNull(@Nonnull ECModElements elements, @Nullable Object getter, @Nonnull Class<? extends T> objType) {
        Map<String, Object> getterMap = ObjHelper.getAnnotationMap(getter);
        if (getterMap == null || !getterMap.containsKey("value") || "".equals(getterMap.get("name"))) {
            return null;
        }
        return getter(elements, getter, objType);
    }
        ...
        if (optional.isPresent()) {
            try {
                ...
                if ("<init>".equals(methodName)) {
                    ...
                    java.lang.reflect.Method m = ReflectUtils.findMethod(aClass, methodName, methodName, argTypes);
