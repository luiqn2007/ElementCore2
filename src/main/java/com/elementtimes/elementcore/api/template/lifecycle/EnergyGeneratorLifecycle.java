...
public class EnergyGeneratorLifecycle<T extends TileEntity> implements IMachineLifecycle {
    ...
    public interface IStrategy {
         ...
         * 仅向 East 方向发送
         */
        IStrategy ONLY_EAST = context -> sendTo(getSource(context, Direction.EAST), getTarget(context, Direction.EAST, Direction.WEST));

        /**
         * 仅向 West 方向发送
         */
        IStrategy ONLY_WEST = context -> sendTo(getSource(context, Direction.WEST), getTarget(context, Direction.WEST, Direction.EAST));

        /**
         * 仅向 North 方向发送
         */
        IStrategy ONLY_NORTH = context -> sendTo(getSource(context, Direction.NORTH), getTarget(context, Direction.NORTH, Direction.SOUTH));

        /**
         * 仅向 South 方向发送
         */
        IStrategy ONLY_SOUTH = context -> sendTo(getSource(context, Direction.SOUTH), getTarget(context, Direction.SOUTH, Direction.NORTH));
         ...
         */
        IStrategy AVERAGE_MINIMUM = context -> {
            ArrayList<Pair<IEnergyStorage, IEnergyStorage>> sourceAndTargets = getSourceAndTargets(context, false, false);
            if (!sourceAndTargets.isEmpty()) {
                int energy = getSourceHandler(context).getEnergyStored() / sourceAndTargets.size();
         ...
         */
        IStrategy AVERAGE_STORED = context -> {
         ...
         */
        static IEnergyStorage getSource(EnergyGeneratorLifecycle<? extends TileEntity> context, Direction facing) {
         ...
         * @return 输出目标
         */
        static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, Direction facing) {
         ...
         * @return 输出目标
         */
        static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, Direction facing, Direction opposite) {
            TileEntity te = context.getTe();
            if (!te.hasWorld()) {
                return null;
            }
