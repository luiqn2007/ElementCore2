...
public class EnergyGeneratorLifecycle<T extends TileEntity> implements IMachineLifecycle {
    ...
    public interface IStrategy {
         ...
         * 仅向 East 方向发送
         */
        IStrategy ONLY_EAST = context -> sendTo(getSource(context, EnumFacing.EAST), getTarget(context, EnumFacing.EAST, EnumFacing.WEST));

        /**
         * 仅向 West 方向发送
         */
        IStrategy ONLY_WEST = context -> sendTo(getSource(context, EnumFacing.WEST), getTarget(context, EnumFacing.WEST, EnumFacing.EAST));

        /**
         * 仅向 North 方向发送
         */
        IStrategy ONLY_NORTH = context -> sendTo(getSource(context, EnumFacing.NORTH), getTarget(context, EnumFacing.NORTH, EnumFacing.SOUTH));

        /**
         * 仅向 South 方向发送
         */
        IStrategy ONLY_SOUTH = context -> sendTo(getSource(context, EnumFacing.SOUTH), getTarget(context, EnumFacing.SOUTH, EnumFacing.NORTH));

        /**
         * 仅向空机器发送能量
         */
        IStrategy ONLY_EMPTY = context -> {
         ...
         */
        IStrategy AVERAGE_MINIMUM = (IStrategy) context -> {
            ArrayList<Pair<IEnergyStorage, IEnergyStorage>> sourceAndTargets = getSourceAndTargets(context, false, false);
            if (!sourceAndTargets.isEmpty()) {
                int energy = getSourceHandler(context).getEnergyStored() / sourceAndTargets.size();
         ...
         */
        IStrategy AVERAGE_STORED = (IStrategy) context -> {
         ...
         */
        IStrategy EMPTY_SEQUENTIAL = context -> {
         ...
         */
        static IEnergyStorage getSource(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing) {
         ...
         */
        static IEnergyStorage getSourceHandler(EnergyGeneratorLifecycle<? extends TileEntity> context) {
            TileEntity te = context.getTe();
            if (te instanceof ITileEnergyHandler) {
                return ((ITileEnergyHandler) te).getEnergyHandler();
            } else {
                return te.getCapability(CapabilityEnergy.ENERGY, null);
            }
        }
         ...
         * @return 输出目标
         */
        static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing) {
         ...
         * @return 输出目标
         */
        static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing, EnumFacing opposite) {
            TileEntity te = context.getTe();
            if (!te.hasWorld()) {
                return null;
            }
            ...
            if (teTarget instanceof ITileEnergyHandler) {
                ...
                return teTarget.getCapability(CapabilityEnergy.ENERGY, opposite);
            } else {
                return null;
            }
        }
         ...
         */
        static ArrayList<Pair<IEnergyStorage, IEnergyStorage>> getSourceAndTargets(EnergyGeneratorLifecycle<? extends TileEntity> context, boolean emptyFirst, boolean sort) {
        ...
        static void sendTo(int energy, IEnergyStorage source, IEnergyStorage target) {
        ...
        static void sendTo(IEnergyStorage source, IEnergyStorage target) {
