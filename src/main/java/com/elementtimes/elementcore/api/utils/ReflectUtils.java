...
public class ReflectUtils {
     ...
     * 只能获取静态值
     */
    public static <T> Optional<T> getFromAnnotated(@Nonnull AnnotatedElement holder, @Nullable T defaultValue, Class<? extends T> type, Logger logger) {
     ...
     */
    public static <T> Optional<T> create(@Nonnull Class<?> aClass, @Nullable Class<? extends T> type, Logger logger) {
        try {
            if (type == aClass || type == null || classCheck(type, aClass)) {
     ...
     */
    public static <T> Optional<T> create(@Nonnull String className, Object[] params, Class<? extends T> type, Logger logger) {
     ...
     * @param aClass 要创建的对象类
     */
    public static <T> Optional<T> create(@Nonnull Class<?> aClass, Object[] params, Class<? extends T> type, Logger logger) {
     ...
     * @param className 要创建的对象类的全类名
     */
    public static <T> Optional<T> create(@Nonnull String className, Class<? extends T> type, Logger logger) {
        if (logger == null) {
     ...
     */
    public static <T> Optional<T> create(@Nonnull Constructor<?> constructor, Class<? extends T> type, Logger logger) {
        if (logger == null) {
            logger = LogManager.getLogger();
        }
        constructor.setAccessible(true);
     ...
     * @param <T> 变量类型
     */
    public static <T> Optional<T> get(@Nonnull Field field, @Nullable Object object, @Nullable T defaultValue, boolean setIfNull, @Nonnull Class<? extends T> type, Logger logger) {
        T obj = null;
        ...
        if (obj == null || !classCheck(type, obj.getClass())) {
            // 尝试根绝类型获取值
     ...
     * @return 尝试获取成员的结果
     */
    public static <T> Optional<T> get(@Nonnull Class<?> clazz, @Nonnull String fieldName, @Nullable Object object, @Nonnull Class<? extends T> type, Logger logger) {
        try {
            // 静态成员
    ...
    private static Object get(Field field, Class<?> type, Object obj) throws IllegalAccessException {
        if (type == int.class) {
     ...
     * @param <T> 方法类型
     */
    public static <T> Optional<T> invoke(@Nonnull Method method, @Nullable Object object, @Nullable Class<? extends T> type, Logger logger, Object... args) {
     ...
     * @param <T> 方法类型
     */
    public static <T> Optional<T> invoke(@Nonnull String methodName, @Nonnull Class<?> holder, @Nullable Object object, @Nullable Class<? extends T> type, Logger logger) {
     ...
     * @param <T> 方法类型
     */
    public static <T> Optional<T> invoke(@Nonnull String methodName, @Nonnull Class<?> holder, @Nullable Object object, @Nullable Class<? extends T> type, Logger logger, Object[] args, Class<?>... argTypes) {
        for (Method declaredMethod : holder.getDeclaredMethods()) {
     ...
     */
    public static void set(@Nonnull Field field, @Nullable Object value, @Nullable Object object, Logger logger) {
        try {
    ...
    public static void setFinalField(Field field, Object newValue) throws NoSuchFieldException, IllegalAccessException {
        setFinalField(null, field, newValue);
    }

    public static void setFinalField(Object object, Field field, Object newValue) throws NoSuchFieldException, IllegalAccessException {
    ...
    public static Method findMethod(Class<?> aClass, String name, String name2, Class<?>... types) {
        try {
            return aClass.getDeclaredMethod(name, types);
        } catch (Exception e0) {
            try {
                return aClass.getDeclaredMethod(name2, types);
            } catch (Exception e1) {
                try {
                    return aClass.getMethod(name, types);
                } catch (Exception e2) {
                    try {
                        return aClass.getMethod(name2, types);
                    } catch (Exception e3) {
                        return null;
                    }
                }
    ...
    public static boolean classCheck(Class<?> parent, Class<?> child) {
        return classCheck(parent, child, int.class    , Integer.class)
                || classCheck(parent, child, char.class   , Character.class)
                || classCheck(parent, child, short.class  , Short.class)
                || classCheck(parent, child, byte.class   , Byte.class)
                || classCheck(parent, child, boolean.class, Boolean.class)
                || classCheck(parent, child, float.class  , Float.class)
                || classCheck(parent, child, double.class , Double.class)
                || classCheck(parent, child, long.class   , Long.class)
                || parent.isAssignableFrom(child);
    }

    private static boolean classCheck(Class<?> parent, Class<?> child, Class<?> c0, Class<?> c1) {
        if (parent == c0 || parent == c1) {
            return child == c0 || child == c1;
        }
        return false;
    }
}
